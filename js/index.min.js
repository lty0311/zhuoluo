...
function preload() {
	...
	if (pageIndex > 0 && pageIndex < $("body>section").length) selector = $("body>section").eq(pageIndex);
	preloadCount = selector.length;
...
function pageLoad() {
	...
	for (var i = 0; i < $(".video .swiper-slide").length; i++) $(".video .guide").append('<a></a>');
...
function initLayout() {
	...
	if ($(window).width() > 1000) {
		...
		var width = items.outerWidth() * items.length + itemMarginLeft * 2 * items.length + 4 * (items.length - 1) + 2;
...
function pageSwitching() {
	...
	if (pageIndex >= $("body>section").length) {
		pageIndex = $("body>section").length - 1;
		stopSwitch = false;
		return
	}
	var lock = false;
